...
```{r perc-atheist-us12}
## proportion of atheists in US 2012
us12 %>% summarise(n = n(), n_atheist = sum(response == "atheist"), prop = mean(response == "atheist"))

cat("Proportion atheist (US 2012):", round(mean(us12$response == "atheist"),4), "\n")
...
## Compute margin of error from inference output (us12 CI)
ci_us12 <- inference(y = response, data = us12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
ci_us12

me <- (ci_us12$upper - ci_us12$lower)/2
...
## Choose two countries, e.g., Spain and Russia (if present)
spain12 <- atheism %>% filter(nationality == "Spain", year == "2012")
russia12 <- atheism %>% filter(nationality == "Russia", year == "2012")

ci_spain <- inference(y = response, data = spain12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
ci_russia <- inference(y = response, data = russia12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")

ci_spain
ci_russia

cat("Spain ME:", round((ci_spain$upper - ci_spain$lower)/2,4), "\n")
cat("Russia ME:", round((ci_russia$upper - ci_russia$lower)/2,4), "\n")
...
```{r spain-05-12}
## Test whether Spain changed between 2005 and 2012
spain <- atheism %>% filter(nationality == "Spain")
table(spain$year)
## ensure year is treated as categorical to avoid warnings
spain <- spain %>% mutate(year = as.factor(year))
inference(y = response, x = year, data = spain, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", null = 0, alternative = "twosided")
...
```{r us-05-12}
## Test whether US changed between 2005 and 2012
us <- atheism %>% filter(nationality == "United States")
table(us$year)
## ensure year is treated as categorical to avoid warnings
us <- us %>% mutate(year = as.factor(year))
inference(y = response, x = year, data = us, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", null = 0, alternative = "twosided")
...
```{r type1}
## Expected number of false positives (Type I errors) among 39 countries
expected_false_pos <- 39 * 0.05
expected_false_pos

cat("Expected number of detections by chance (alpha=0.05):", expected_false_pos, "\n")
...
```{r sample-size}
## Required sample size for ME <= 0.01 with 95% confidence, conservative p=0.5
z95 <- qnorm(0.975)
ME <- 0.01
p <- 0.5
n_required <- (z95^2 * p * (1-p)) / (ME^2)
ceiling(n_required)

cat("Required sample size (conservative):", ceiling(n_required), "\n")
