...
## Visualize the sample (n = 50) and compute sample summary statistics
ggplot(samp1, aes(x = area)) +
  geom_histogram(binwidth = 250, color = "black", fill = "lightblue") +
  ggtitle("Sample (n = 50) distribution of area") +
  xlab("area (sq ft)")

# sample summary statistics (note labels indicate these are sample values)
samp1 %>%
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
## How many elements and visualize the sampling distribution (from earlier code)
cat("Number of elements in sample_means50:", nrow(sample_means50), "\n")

ggplot(sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20, color = "black", fill = "lightgreen") +
  geom_vline(xintercept = mean(sample_means50$x_bar), color = "red", linetype = "dashed") +
  ggtitle("Sampling distribution of sample means (n = 50)") +
  xlab("sample mean of area")

sample_means50 %>% summarise(mean = mean(x_bar), se = sd(x_bar))
...
# type your code for the Exercise here, and Run Document
## Create 25 sample means from samples of size 10 and show the result
sample_means_small <- ames %>%
  rep_sample_n(size = 10, reps = 25, replace = TRUE) %>%
  summarise(x_bar = mean(area))

# print the sampling distribution and its size
sample_means_small
cat("Number of observations in sample_means_small:", nrow(sample_means_small), "\n")
...
## Report the number of elements in sample_means_small
...
## Take a random sample of size 50 from price and report the sample mean
samp_price50 <- ames %>% sample_n(size = 50)
samp_price50 %>% summarise(x_bar = mean(price))
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
## Simulate sampling distribution of sample means for price (n = 50, reps = 5000)
sample_means50 <- ames %>%
  rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>%
  summarise(x_bar = mean(price))

ggplot(sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 5000, color = "black", fill = "salmon") +
  geom_vline(xintercept = mean(sample_means50$x_bar), color = "blue", linetype = "dashed") +
  ggtitle("Sampling distribution of mean price (n = 50, reps = 5000)") +
...
# type your code for this Exercise here, and Run Document
## Change sample size to 150 and compute sampling distribution (reps = 5000)
...
ggplot(sample_means150, aes(x = x_bar)) +
  ...
  geom_vline(xintercept = mean(sample_means150$x_bar), color = "blue", linetype = "dashed") +
  ggtitle("Sampling distribution of mean price (n = 150, reps = 5000)") +
  xlab("sample mean of price")

sample_means150 %>% summarise(mean = mean(x_bar), se = sd(x_bar))
...
# type your code for this Exercise here, and Run Document
## Take a sample of size 15 and compute the sample mean for price
samp_price15 <- ames %>% sample_n(size = 15)
samp_price15 %>% summarise(x_bar = mean(price))
...
# type your code for this Exercise here, and Run Document
## Simulate sampling distribution for price with n = 15 and reps = 2000
sample_means15 <- ames %>%
  rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>%
  summarise(x_bar = mean(price))

ggplot(sample_means15, aes(x = x_bar)) +
  geom_histogram(binwidth = 10000, color = "black", fill = "palegreen") +
  geom_vline(xintercept = mean(sample_means15$x_bar), color = "blue", linetype = "dashed") +
  ggtitle("Sampling distribution of mean price (n = 15, reps = 2000)") +
  xlab("sample mean of price")

sample_means15 %>% summarise(mean = mean(x_bar), se = sd(x_bar))

# population mean for reference
cat("Population mean of price:", mean(ames$price), "\n")
...
# type your code for this Exercise here, and Run Document
## Increase sample size to 150 and compute sampling distribution (reps = 2000)
sample_means150 <- ames %>%
  rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>%
  summarise(x_bar = mean(price))

ggplot(sample_means150, aes(x = x_bar)) +
  geom_histogram(binwidth = 5000, color = "black", fill = "lightcoral") +
  geom_vline(xintercept = mean(sample_means150$x_bar), color = "blue", linetype = "dashed") +
  ggtitle("Sampling distribution of mean price (n = 150, reps = 2000)") +
  xlab("sample mean of price")
...
## Compare the two sampling distributions (n = 50 vs n = 150)
stats_50 <- sample_means50 %>% summarise(mean = mean(x_bar), sd = sd(x_bar))
stats_150 <- sample_means150 %>% summarise(mean = mean(x_bar), sd = sd(x_bar))

cat("n = 50 -> mean:", round(stats_50$mean, 2), " SD:", round(stats_50$sd, 2), "\n")
cat("n = 150 -> mean:", round(stats_150$mean, 2), " SD:", round(stats_150$sd, 2), "\n")

# overlay density plots for comparison
df50 <- data.frame(x = sample_means50$x_bar, sample = "n50")
df150 <- data.frame(x = sample_means150$x_bar, sample = "n150")
df_all <- bind_rows(df50, df150)

ggplot(df_all, aes(x = x, fill = sample)) +
  geom_density(alpha = 0.4) +
  ggtitle("Density comparison of sampling distributions: n=50 vs n=150") +
  xlab("sample mean of price")
