...
```{r describe-sample}
## Visualize and summarise the sample of area (n = 60)
ggplot(samp, aes(x = area)) +
  geom_histogram(binwidth = 250, color = "black", fill = "lightblue") +
  ggtitle(paste0("Histogram of sampled area (n = ", n, ")")) +
  xlab("area (sq ft)")

samp %>% summarise(sample_mean = mean(area), sample_median = median(area),
...
## Check whether the CI computed earlier contains the true population mean
ci_single <- samp %>%
  summarise(lower = mean(area) - z_star_95 * (sd(area) / sqrt(n)),
            upper = mean(area) + z_star_95 * (sd(area) / sqrt(n)))
...
cat("Population mean (params$mu):", params$mu, "\n")
cat("Does the interval include the population mean?:",
    ci_single$lower < params$mu & ci_single$upper > params$mu, "\n")
...
## Critical value for 99% two-sided CI
...
```{r plot-99-perc-cis}
## Construct 99% confidence intervals from the previously computed 95% CIs
z_star_99 <- qnorm(0.995)

# use the existing 'ci' data frame (which holds 95% CI bounds)
ci_99 <- ames %>%
  rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
...
ggplot(data = ci99_data, aes(x = ci_bounds, y = ci_id, group = ci_id, color = capture_mu)) +
  geom_point(size = 2) +
  geom_line() +
  geom_vline(xintercept = params$mu, color = "darkgray") +
  ggtitle("99% confidence intervals (50 samples)") +
  xlab("area (sq ft)")

# proportion that include the true mean
prop_include <- mean(ci_99$capture_mu == "yes")
cat("Proportion of 99% CIs that include the true mean:", prop_include, "\n")
